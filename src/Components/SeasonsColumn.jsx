// // import HyphenIcon from "../assets/images/-.svg";

// // function SeasonsColumn() {
// //     return (
// //       <>
// //         <div className="seasonsColumn">
// //             <div className="seasonsColumn_header">
// //                 <div className="seasonsColumn_header_title">Seasons of Splendor</div>
// //                 <div className="seasonsColumn_header_subtitle">
// //                     <img src={HyphenIcon} className="seasonsColumn_header_subtitle_hyphen" alt="-" />
// //                     <p className="seasonsColumn_header_subtitle_content">A Yearlong Tapestry of Indian Celebrations at Coconest Eco Resort.</p>
// //                 </div>
// //             </div>

// //             <div className="seasonsColumn_body">
// //                 <div className="seasonsColumn_row">
// //                     <div className="seasonsColumn_column">
// //                         <p className="seasonsColumn_column_heading">December</p>
// //                         <ul className="seasonsColumn_column_content">
// //                             <li>Christmas Celebration</li>
// //                             <li>New Year&apos;s Eve Gala</li>
// //                         </ul>
// //                     </div>
// //                     <div className="seasonsColumn_column">
// //                         <p className="seasonsColumn_column_heading">January</p>
// //                         <ul className="seasonsColumn_column_content">
// //                             <li>New Year&apos;s Celebrations and Festivals</li>
// //                             <li>Republic Day Celebration</li>
// //                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">February</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Basant Panchamin</li>
//                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">March</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Holi</li>
//                             <li>International Women&apos;s Day Celebration</li>
//                         </ul>
//                     </div>

//                 </div>
//                 <div className="seasonsColumn_row">
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">April</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Mewar Festival</li>
//                             <li>Ram Navami</li>
//                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">May</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Rajasthan Diwas</li>
//                             <li>International Museum Day</li>
//                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">June</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Kabir Jayanti</li>
//                             <li>Summer Solstice Festivals</li>
//                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">July</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Guru Purnima</li>
//                         </ul>
//                     </div>

//                 </div>
//                 <div className="seasonsColumn_row">
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">August</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Independence Day Celebration</li>
//                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">September</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Ganesh Chaturthi</li>
//                             <li>Onam</li>
//                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">October</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Navaratri and Durga Puja</li>
//                             <li>Gandhi Jayanti</li>
//                             <li>Diwali Festival</li>
//                         </ul>
//                     </div>
//                     <div className="seasonsColumn_column">
//                         <p className="seasonsColumn_column_heading">November</p>
//                         <ul className="seasonsColumn_column_content">
//                             <li>Guru Nanak Jayanti </li>
//                             <li>Pushkar Camel Fair</li>
//                         </ul>
//                     </div>

//                 </div>
//             </div>
//             <div className="seasonsColumn_footer">
//                 <button className="seasonsColumn_footer_button">KNOW MORE DETAILS</button>
//             </div>
            
//         </div>
        
//       </>
//     )
//   }
  
//   export default SeasonsColumn;